...
int	_looking_direction(t_src *src)
{
	...
	tmp = src->xm;
	mlx_get_mouse_pos(src->mlx, &src->xm, &src->ym);
	if (mlx_is_key_down(src->mlx, MLX_KEY_LEFT) || src->xm < tmp)
	{
		src->pa = normalize_angle(src->pa - P_ANGLE_ROTATION);
		rendring(src);
	}
	else if (mlx_is_key_down(src->mlx, MLX_KEY_RIGHT) || src->xm > tmp)
	{
		src->pa = normalize_angle(src->pa += P_ANGLE_ROTATION);
		rendring(src);
	}
